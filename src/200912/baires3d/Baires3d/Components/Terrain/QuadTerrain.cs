...
namespace ar3d
{
    ...
    public class QuadTerrain
    {
        ...
        public QuadTerrain(GraphicsDevice g, Effect terrainShader, Texture2D heightMap, int squareSize, int vertexBufferSize, float scale, float height)
        {
            ...
            //Initialise the link to the graphics device.
            graphics = g;

            //Initialise the link to the shader, and activate a few reference variables.
            ...
            */
            ...
            for (int i = 0; i < numberOfVBs + 1; i++)
            {
                ...
                allVBs[i] = new VertexBuffer(graphics, (VBsize + 1) * (VBsize + 1) * VertexPosition.SizeInBytes, BufferUsage.WriteOnly);
            ...
            {
                allIBs[l] = new DynamicIndexBuffer(graphics, (allVertices[l].Length) * 32, BufferUsage.WriteOnly, IndexElementSize.ThirtyTwoBits);
            }
        ...
        private void RecursiveParentStatus(QuadNode qNode)
        {
            if (qNode.NodeDepth > 0)
            {
                ...
                if (qNode.status == 1)
                {
                    ...
                    {
                        MessageBox.Show("debug problem1");
                    }
                }
                else
                {
                    ...
                    {
                        MessageBox.Show("debug problem2");
                    }
                }
        ...
        public void UpdateTerrain(Vector3 CameraPoint, BoundingFrustum CameraFrustrum, float LODLevel)
        {
#if DEBUG
            debugTimer.Start();
#endif
        ...
        public void DrawTerrain(GraphicsDevice device)
        {
            ...
            foreach (EffectPass pass in TerrainShader.CurrentTechnique.Passes)
            {
                ...
                for (int i = 0; i < allVertices.Length; i++)
                {
                    ...
                    device.Vertices[0].SetSource(allVBs[i], 0, VertexPosition.SizeInBytes);
                    if (renderedindices[i] > 0)
                    {
