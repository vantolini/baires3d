...
namespace b3d
{
    ...
    /// </summary>
    public static class TriangulatorGood
    {
        #region Fields
        ...
        public static Vector2[] CutHoleInShape(Vector2[] shapeVerts, Vector2[] holeVerts)
        {
            ...
            Vector2 vx0 = new Vector2(1, 0);
            Vector2 I = rightMostHoleVertex.Position + vx0*closestPoint.Value;
            Vertex P = (closestSegment.A.Position.X > closestSegment.B.Position.X)
                           ? closestSegment.A
                           : closestSegment.B;

            //construct triangle MIP
            TTriangle mip = new TTriangle(rightMostHoleVertex, new Vertex(I, 1), P);
            ...
            if (interiorReflexVertices.Count > 0)
            {
                ...
                foreach (Vertex v in interiorReflexVertices)
                {
                    ...
                    double dot = Vector2.Dot(new Vector2(1, 0), d);
        ...
        private static void ClipNextEar(ICollection<TTriangle> triangles)
        {
            //find the triangle
            ...
            triangles.Add(new TTriangle(ear, next, prev));
        ...
        private static bool IsEar(Vertex c)
        {
            ...
            foreach (Vertex t in reflexVertices)
            {
                ...
                if (TTriangle.ContainsPoint(p, c, n, t))
                {
