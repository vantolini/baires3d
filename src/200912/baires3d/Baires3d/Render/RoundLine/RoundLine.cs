...
namespace b3d
{
    ...
    [6]: "Glow"*/
            ...
            foreach (RoundLine roundLine in roundLines){
                /*m = new Matrix( (float)Math.Cos(roundLine.Theta), 0, (float)Math.Sin(roundLine.Theta), 1,
                   (float)Math.Sin(roundLine.Theta), 0, (float)Math.Cos(roundLine.Theta), 1,          1,0,1,1,          roundLine.P0.X, 1, roundLine.P0.Y, 1 );*/
                /*rot = 
                    Matrix.CreateRotationZ(-(float) Math.Cos(roundLine.Theta))
                    *
                      Matrix.CreateRotationY((float)Constants.ar3d.form.nmrNear.Value)
                    //Matrix.CreateRotationY((float) Math.Sin(roundLine.Theta))
                      
                      ;

                trans = Matrix.CreateTranslation(roundLine.P0.X, 0, roundLine.P0.Y);
                m = Matrix.Identity*rot*trans;
                */
    //Matrix.CreateRotationX((float)Math.Cos(roundLine.Theta))
